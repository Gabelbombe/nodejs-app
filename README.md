...
#### Complete Code
 ...
 - Terraform Repository, submoduled into the terraform folder.
...
Ways that I can improve this in the long run...
 - `Makefile` as central keypoint for deploys inside of `Jenkinsfile`
 - Third layer of testing with infra, current windows includes
    - `Jenkinsfile` Code centric testing for unit tests
    - `Packer` testing with ServerSpec (short demo)
 - True region agnosticity, still relies on region static in `packer/packer.json`
 - Automated GitHook allocation (Needs live Jenkins box first)
 - Testing for static deploy/build files, RE: Jenkinsfile, Dockerfile, TFFile and Packer
